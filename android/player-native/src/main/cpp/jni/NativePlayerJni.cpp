...
extern "C" JNIEXPORT void JNICALL
Java_com_reallive_player_NativePlayer_nativePause(JNIEnv*, jobject, jlong handle) {
    auto* controller = fromHandle(handle);
    if (!controller) return;
    RL_JNI_LOGI("nativePause controller=%p", static_cast<void*>(controller));
    controller->pause();
}

extern "C" JNIEXPORT void JNICALL
Java_com_reallive_player_NativePlayer_nativeResume(JNIEnv*, jobject, jlong handle) {
    auto* controller = fromHandle(handle);
    if (!controller) return;
    RL_JNI_LOGI("nativeResume controller=%p", static_cast<void*>(controller));
    controller->resume();
}
